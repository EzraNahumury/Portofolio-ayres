...
export function WhatYouCanDo() {
  ...
  const visible = showAll
    ? whatYouCanDo.cards
    : whatYouCanDo.cards.slice(0, whatYouCanDo.visibleCount);
  const remaining = whatYouCanDo.cards.length - whatYouCanDo.visibleCount;
      ...
      <div className="mx-auto max-w-[1400px] px-6 lg:px-10">
        ...
        <div className="wycd-grid mt-20 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 lg:gap-7">
          {visible.map((c) => {
            return (
              <TiltCard key={c.name}>
                {/* Plain <a>, not next/link — this must escape the
                    /normal-mode basePath and land on the site-root project
                    detail page. */}
                <a
                  href={`/project.html?p=${c.slug}&theme=dark`}
                ...
                >
                  ...
                  {/* Visual: animated node mesh, unique per project */}
                  <div
        ...
        {remaining > 0 && (
          <div className="mt-14 flex justify-center">
            <button
              ...
              className="group inline-flex items-center gap-2.5 rounded-full border border-border-strong px-6 py-2.5 text-sm font-medium text-fg transition-colors hover:bg-white/[0.04]"
              style={{
            ...
            >
              <span>
                {showAll
                  ? whatYouCanDo.showLessLabel
                  : `${whatYouCanDo.showMoreLabel} (${remaining} more)`}
              </span>
              <ChevronDown
