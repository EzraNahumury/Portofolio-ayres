...
export function ReputationBar() {
  ...
  useEffect(() => {
    ...
    function frame(t: number) {
      ...
      const score = Math.floor(easeOutCubic(fillPhase) * TARGET);
      ...
      const fillW = (score / 1000) * barW;
      const grad = ctx!.createLinearGradient(padX, 0, padX + barW, 0);
      grad.addColorStop(0, "hsla(28, 80%, 60%, 0.9)");
      grad.addColorStop(0.33, "hsla(220, 30%, 75%, 0.9)");
      grad.addColorStop(0.66, "hsla(44, 90%, 60%, 0.95)");
      grad.addColorStop(1, "hsla(200, 80%, 70%, 0.95)");
      ctx!.fillStyle = grad;
      ...
      for (let i = 0; i < segs; i++) {
        ...
        const reached = score >= (i === 0 ? 1 : TIERS[i - 1].max);
