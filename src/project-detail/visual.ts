/**
 * Per-project animated canvas scenes for the detail pages — ported from
 * normal-mode/components/visuals/project-visual.tsx (framework-free here).
 * Each slug gets its own mini-scene on a dark "monitor" canvas (dot grid,
 * purple/pink/blue accents) and reacts to the cursor. The dark canvas works
 * in both themes: the retro page frames it like a CRT screen.
 *
 * Keep the drawer functions in sync with the normal-mode copy when editing.
 */
...
/**
 * Mounts the animated scene for `kind` into `host` (host must be
 * position:relative). Vanilla port of the React wrapper: resilient RAF loop
 * (a throwing frame never kills the animation), IntersectionObserver pause
 * when offscreen, DPR-aware sizing, cursor interaction.
 */
export function mountVisual(host: HTMLElement, kind: string): void {
  ...
  function frame(now: number) {
    // schedule first so a drawing exception can never kill the loop
  ...
  sizeUp();
  new ResizeObserver(sizeUp).observe(host);
  requestAnimationFrame(frame);
}
